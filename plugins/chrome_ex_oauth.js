...
//      callback(xhr.responseText, xhr);
      callback.apply(this, [xhr.responseText, xhr, type]);
    }
  },syn);
};
 ...
 */
ChromeExOAuth.sendRequest = function(method, url, headers, body, callback,syn) {
...
ChromeExOAuth.prototype.initOAuthFlow = function(callback) {
  if (!this.hasToken()) {
    ...
    if (params['chromeexoauthcallback'] == 'true') {
      ...
      var request_params = {
//  		'url_callback':'chrome-extension://afcbojmcaaaohbpcclhhjcidbcbebfod/main.html',
...
ChromeExOAuth.prototype.onRequestToken = function(callback, xhr) {
  if (xhr.readyState == 4) {
    if (xhr.status == 200) {
//      var params = ChromeExOAuth.formDecode(xhr.responseText); //Can't be used in new chrome any more/Claud mark
      var params = JSON.parse(xhr.responseText);	//Claud Here
	  chrome.extension.getBackgroundPage().console.log(params['oauth_token']);
	  //alert('');
	  ...
	  alert("Fetching request token failed. Status ");
    }
  }
};
